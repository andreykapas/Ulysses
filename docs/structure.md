...
- **Многоязычность интерфейса (ru / en / pl).** Сейчас видимый текст вшит в
  `index.html` (подзаголовок, меню, заголовки секций). Для трёх языков это не
  годится. Решение: вынести надписи UI в переводы (напр. `content/ui/ru.json`,
  `content/ui/en.json`), а язык подставлять через JS. Тот же принцип «контент
  отдельно от кода». Разобрать после первого JS-урока.
